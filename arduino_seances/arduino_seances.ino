/*
 * SYSTÈME DE POINTAGE PAR SÉANCES - ESP32
 * 
 * Logique d'accès:
 * - Empreinte inconnue → porte fermée
 * - Empreinte connue:
 *   * 8h00-8h10 → porte ouvre (présent ou retard)
 *   * Après 8h10 → porte fermée (trop tard)
 * - Séances suivantes: élève absent précédemment → porte fermée SAUF si billet
 * - Chaque séance: S1 (8h-9h), S2 (9h-10h), ..., S8 (16h-17h)
 */
...
// Vérifie si l'étudiant est nouveau (inscrit aujourd'hui)
bool estNouveauEtudiant(String etuId) {
  HTTPClient http;
  WiFiClientSecure client;
  client.setInsecure();
  
  String date = getDate();
  String url = "https://" + String(firebaseHost) + "/etudiants_nouveaux/" + date + "/" + etuId + ".json";
  ...
  // Si on trouve des données, l'étudiant a été inscrit aujourd'hui
...
bool etaitAbsentSeancePrecedente(String etuId, String classeId, int seanceActuelleIdx) {
  ...
  // NOUVEAU: Si l'étudiant est nouveau (inscrit aujourd'hui), ne pas vérifier les absences passées
  if (estNouveauEtudiant(etuId)) {
    Serial.println("✅ " + etuId + " est nouveau - pas de vérification des absences passées");
    return false;
  }
